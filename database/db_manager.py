...
def _sync_products_excel_safe():
    """Best-effort sync of products.xlsx; nevimport sqlite3
import os
...
def CRITICAL_init_ledger_tables():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS ledger_customers (
            khata_id INTEGER PRIMARY KEY AUTOINCREMENT,
            customer_name TEXT NOT NULL,
            phone_number TEXT, -- Optional: can be left empty and added later
    ...
    # Rebuild the table without that constraint if needed, preserving data.
    cursor.execute("PRAGMA table_info(ledger_customers)")
    cols_info = cursor.fetchall()
    phone_col = next((c for c in cols_info if c[1] == "phone_number"), None)
    if phone_col and phone_col[3] == 1:  # notnull == 1 -> old strict schema
        cursor.execute("ALTER TABLE ledger_customers RENAME TO ledger_customers_old")
        cursor.execute("""
        ...
        """)
        cursor.execute("""
            INSERT INTO ledger_customers (khata_id, customer_name, phone_number, current_wallet_balance, created_at)
            SELECT khata_id, customer_name, phone_number, current_wallet_balance, created_at FROM ledger_customers_old
        """)
        cursor.execute("DROP TABLE ledger_customers_old")

    # Partial unique index: only enforce uniqueness when a phone number is actually provided
    cursor.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS idx_ledger_customers_phone_unique
        ON ledger_customers (phone_number)
...
def add_ledger_customer(name, phone, opening_balance=0.0):
    """Creates a new khata account. Phone is optional (can be added later).
    opening_balance: positive = customer advance/credit with us, negative = udhaar (customer owes us)."""
    import sqlite3
    try:
        ...
        if opening_balance != 0:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (khata_id, action_type, abs(opening_balance), opening_balance, "Previous Balance (Opening)"))
...
def update_ledger_customer(khata_id, name, phone):
    """Updates only the customer name and phone number for a khata account. Phone is optional."""
    import sqlite3
    try:
        phone = (phone or "").strip()
        phone_value = phone if phone else None
...
def import_ledger_customers_bulk(rows):
    ...
    rows: list of (name, phone, opening_balance) tuples. Phone is OPTIONAL — can be
    left empty and added later from the app.
    Skips rows with no name, and rows whose phone number duplicates an existing
    customer's phone (only checked when a phone number is actually provided).
    Opening balance becomes the starting wallet balance plus an
    ADVANCE_DEPOSIT/initial log entry if non-zero.
...
init_db()er crashes the calling operation."""
    try:
        from modules.excel_sync import sync_products_excel
        sync_products_excel()
    except Exception:
